...
class PyHeat(hass.Hass):
    ...
    def register_services(self):
        """Register all pyheat.* service handlers with Home Assistant."""
        self.log("Registering service handlers...")
        
        # Register each service
        self.register_service("pyheat/override", self.svc_override)
        self.register_service("pyheat/boost", self.svc_boost)
        self.register_service("pyheat/cancel_override", self.svc_cancel_override)
        self.register_service("pyheat/set_mode", self.svc_set_mode)
        self.register_service("pyheat/set_default_target", self.svc_set_default_target)
        self.register_service("pyheat/reload_config", self.svc_reload_config)
        self.register_service("pyheat/get_schedules", self.svc_get_schedules)
        self.register_service("pyheat/get_rooms", self.svc_get_rooms)
        self.register_service("pyheat/replace_schedules", self.svc_replace_schedules)
        
        self.log("Service handlers registered")
    ...
    def svc_override(self, namespace, domain, service, kwargs):
        ...
        if room is None:
            self.log("pyheat.override: 'room' argument is required", level="ERROR")
            return {"success": False, "error": "room argument is required"}
        
        if target is None:
            self.log("pyheat.override: 'target' argument is required", level="ERROR")
            return {"success": False, "error": "target argument is required"}
        
        if minutes is None:
            self.log("pyheat.override: 'minutes' argument is required", level="ERROR")
        ...
        try:
            ...
            self.log(f"pyheat.override: invalid argument types: {e}", level="ERROR")
            return {"success": False, "error": f"invalid argument types: {e}"}
        
        if minutes <= 0:
            self.log("pyheat.override: 'minutes' must be positive", level="ERROR")
        ...
        try:
            ...
            if self.entity_exists(override_entity):
                self.call_service("input_number/set_value", entity_id=override_entity, value=target)
            
            # Start override timer
            timer_entity = C.HELPER_ROOM_OVERRIDE_TIMER.format(room=room)
            if self.entity_exists(timer_entity):
                duration = f"{minutes * 60}"  # Convert to seconds
                self.call_service("timer/start", entity_id=timer_entity, duration=duration)
            
            # Trigger immediate recompute
            self.run_in(lambda kwargs: self.recompute_all("override service"), 1)
            
            return {"success": True, "room": room, "target": target, "minutes": minutes}
        
        except Exception as e:
            self.log(f"pyheat.override failed: {e}", level="ERROR")
    ...
    def svc_boost(self, namespace, domain, service, kwargs):
        ...
        # Validate required arguments
        if room is None:
            self.log("pyheat.boost: 'room' argument is required", level="ERROR")
            return {"success": False, "error": "room argument is required"}
        
        if delta is None:
            self.log("pyheat.boost: 'delta' argument is required", level="ERROR")
            return {"success": False, "error": "delta argument is required"}
        
        if minutes is None:
            self.log("pyheat.boost: 'minutes' argument is required", level="ERROR")
            return {"success": False, "error": "minutes argument is required"}
        ...
        try:
            ...
            self.log(f"pyheat.boost: invalid argument types: {e}", level="ERROR")
            return {"success": False, "error": f"invalid argument types: {e}"}
        
        if minutes <= 0:
            self.log("pyheat.boost: 'minutes' must be positive", level="ERROR")
            return {"success": False, "error": "minutes must be positive"}
        ...
        try:
            ...
            room_mode = self.get_state(mode_entity) if self.entity_exists(mode_entity) else "auto"
            room_mode = room_mode.lower() if room_mode else "auto"
            holiday_mode = self.get_state(C.HELPER_HOLIDAY_MODE) if self.entity_exists(C.HELPER_HOLIDAY_MODE) else "off"
            
            # Resolve current target (without override/boost)
            current_target = None
            if room_mode == "manual":
                setpoint_entity = C.HELPER_ROOM_MANUAL_SETPOINT.format(room=room)
                if self.entity_exists(setpoint_entity):
                    current_target = float(self.get_state(setpoint_entity))
            elif room_mode == "auto":
                current_target = self.get_scheduled_target(room, now, holiday_mode)
            
            if current_target is None:
                self.log(f"pyheat.boost: could not resolve current target for room '{room}'", level="ERROR")
            ...
            override_entity = C.HELPER_ROOM_OVERRIDE_TARGET.format(room=room)
            if self.entity_exists(override_entity):
                self.call_service("input_number/set_value", entity_id=override_entity, value=boost_target)
            ...
            # Trigger immediate recompute
            self.run_in(lambda kwargs: self.recompute_all("boost service"), 1)
            
            return {"success": True, "room": room, "delta": delta, "boost_target": boost_target, "minutes": minutes}
        
        except Exception as e:
            self.log(f"pyheat.boost failed: {e}", level="ERROR")
            return {"success": False, "error": str(e)}
    ...
    def svc_cancel_override(self, namespace, domain, service, kwargs):
        ...
        if room is None:
            self.log("pyheat.cancel_override: 'room' argument is required", level="ERROR")
            return {"success": False, "error": "room argument is required"}
        
        # Validate room exists
        if room not in self.rooms:
            self.log(f"pyheat.cancel_override: room '{room}' not found", level="ERROR")
            return {"success": False, "error": f"room '{room}' not found"}
        
        self.log(f"pyheat.cancel_override: room={room}")
        ...
        try:
            ...
            # Trigger immediate recompute
            self.run_in(lambda kwargs: self.recompute_all("cancel_override service"), 1)
            
            return {"success": True, "room": room}
        
        except Exception as e:
            self.log(f"pyheat.cancel_override failed: {e}", level="ERROR")
            return {"success": False, "error": str(e)}
    ...
    def svc_set_mode(self, namespace, domain, service, kwargs):
        ...
        # Validate required arguments
        if room is None:
            self.log("pyheat.set_mode: 'room' argument is required", level="ERROR")
            return {"success": False, "error": "room argument is required"}
        
        if mode is None:
            self.log("pyheat.set_mode: 'mode' argument is required", level="ERROR")
            return {"success": False, "error": "mode argument is required"}
        
        # Validate mode
        mode = mode.lower()
        if mode not in ["auto", "manual", "off"]:
            self.log(f"pyheat.set_mode: invalid mode '{mode}' (must be auto, manual, or off)", level="ERROR")
            return {"success": False, "error": f"invalid mode '{mode}' (must be auto, manual, or off)"}
        
        # Validate room exists
        if room not in self.rooms:
            self.log(f"pyheat.set_mode: room '{room}' not found", level="ERROR")
            return {"success": False, "error": f"room '{room}' not found"}
        
        self.log(f"pyheat.set_mode: room={room}, mode={mode}")
        ...
        try:
            ...
            if self.entity_exists(mode_entity):
                ...
                self.call_service("input_select/select_option", entity_id=mode_entity, option=mode_display)
            
            # Trigger immediate recompute
            self.run_in(lambda kwargs: self.recompute_all("set_mode service"), 1)
            
            return {"success": True, "room": room, "mode": mode}
        
        except Exception as e:
            self.log(f"pyheat.set_mode failed: {e}", level="ERROR")
            return {"success": False, "error": str(e)}
    ...
    def svc_set_default_target(self, namespace, domain, service, kwargs):
        ...
        # Validate required arguments
        if room is None:
            self.log("pyheat.set_default_target: 'room' argument is required", level="ERROR")
            return {"success": False, "error": "room argument is required"}
        
        if target is None:
            self.log("pyheat.set_default_target: 'target' argument is required", level="ERROR")
            return {"success": False, "error": "target argument is required"}
        ...
        try:
            ...
            self.log(f"pyheat.set_default_target: invalid target type: {e}", level="ERROR")
        ...
        try:
            ...
            self.schedules[room]['default_target'] = target
            ...
            self.log(f"pyheat.set_default_target failed: {e}", level="ERROR")
            return {"success": False, "error": str(e)}

    def svc_reload_config(self, namespace, domain, service, kwargs):
        """Service: pyheat.reload_config - Reload configuration files.
        
        Returns:
            Dict with room_count and schedule_count
        """
        self.log("pyheat.reload_config: reloading configuration")
        
        try:
            self.load_configuration()
            
            # Trigger immediate recompute
            self.run_in(lambda kwargs: self.recompute_all("reload_config service"), 1)
            ...
            self.log(f"Configuration reloaded: {result}")
            return result
        
        except Exception as e:
            self.log(f"pyheat.reload_config failed: {e}", level="ERROR")
            return {"success": False, "error": str(e)}
    ...
    def svc_get_schedules(self, namespace, domain, service, kwargs):
        ...
        """
        self.log("pyheat.get_schedules: retrieving current schedules", level="DEBUG")
        
        try:
            return self.schedules
        except Exception as e:
            self.log(f"pyheat.get_schedules failed: {e}", level="ERROR")
            return {"success": False, "error": str(e)}
    ...
    def svc_get_rooms(self, namespace, domain, service, kwargs):
        ...
        """
        self.log("pyheat.get_rooms: retrieving current rooms", level="DEBUG")
        
        try:
            return self.rooms
        except Exception as e:
            self.log(f"pyheat.get_rooms failed: {e}", level="ERROR")
            return {"success": False, "error": str(e)}
    ...
    def svc_replace_schedules(self, namespace, domain, service, kwargs):
        ...
        # Validate required argument
        if schedule is None:
            self.log("pyheat.replace_schedules: 'schedule' argument is required", level="ERROR")
            return {"success": False, "error": "schedule argument is required"}
        
        if not isinstance(schedule, dict):
            self.log("pyheat.replace_schedules: 'schedule' must be a dict", level="ERROR")
            return {"success": False, "error": "schedule must be a dict"}
        
        self.log("pyheat.replace_schedules: processing request")
        
        try:
            # Validate schedule structure
            total_blocks = 0
            for room_id, room_schedule in schedule.items():
                if 'schedule' in room_schedule and isinstance(room_schedule['schedule'], dict):
                    for day, blocks in room_schedule['schedule'].items():
            ...
            # Trigger immediate recompute
            self.run_in(lambda kwargs: self.recompute_all("replace_schedules service"), 1)
            ...
            self.log(f"Schedules replaced: {result}")
            return result
        
        except Exception as e:
            self.log(f"pyheat.replace_schedules failed: {e}", level="ERROR")
            return {"success": False, "error": str(e)}
